...
test.describe('graph activity', () => {
  ...
  test('run a graph with instructions and node_schema', async ({ page }) => {
    ...
    await graphPage.fillInputField('user_input_proposal', `Should we build a turing complete workflow engine that runs graphs of LLM-backed nodes?`);
    await graphPage.fillInputField('user_input_special_considerations', 'The existence of other workflow engines, possibilities for supporting/monetizing, feasability...');
    await graphPage.submitInputForm();
