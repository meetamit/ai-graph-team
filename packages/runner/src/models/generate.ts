...
function genNumber(s: {
  ...
  if (isFiniteNum(s.exclusiveMinimum)) lo = Math.max(lo, s.exclusiveMinimum! + eps());
  if (isFiniteNum(s.exclusiveMaximum)) hi = Math.min(hi, s.exclusiveMaximum! - eps());

  // Derive a stable fraction based on path depth for nice spreads like 0.25, 0.5, ...
  const denom = 4;
  const idx = (path.length ? (path[path.length - 1].charCodeAt(0) % denom) : 0) + 1;
