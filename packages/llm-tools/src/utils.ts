...
import type { Tool, ToolSettingsSchema } from './index';
...
export function buildToolSettingsSchema(tool: Tool, config: NodeToolConfig | string | undefined): ToolSettingsSchema {
  ...
  const formData = typeof config === 'object' ? config.config : {};
...
export function buildToolConfigSchema(tool: Tool, config: NodeToolConfig | string | undefined): JSONSchema7 {
...
export function buildToolConfig(tool: Tool, rawConfig?: NodeToolConfig): NodeToolConfig | string {
  ...
  const cleaned: NodeToolConfig['config'] = {};
  for (const [fieldName, fieldConfig] of Object.entries(rawConfig?.config || {})) {
    const setting = (configSchema.properties?.[fieldName] as JSONSchema7)?.properties
    const f = { ...fieldConfig }
    ...
    if (Object.keys(f).length > 0) { cleaned[fieldName] = f; }
  }
  for (const fieldName of configSchema.required || []) {
    if (!cleaned[fieldName]) {
      const setting = configSchema.properties?.[fieldName] as JSONSchema7
      cleaned[fieldName] = { value: (setting?.properties?.value as JSONSchema7)?.default };
    }
  }
  const name = tool.id;
  return Object.keys(cleaned).length === 0 ? name : { name, config: cleaned };
}
