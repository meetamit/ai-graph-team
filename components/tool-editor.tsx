...
export default function ToolEditorModal({
  ...
  const handleInteractOutside = useCallback((event: any) => {
    ...
    const _config = typeof config === 'string' ? undefined : config;
    const isDirty = config !== initialConfig && _config !== initialConfig && JSON.stringify(_config) !== JSON.stringify(initialConfig);
    if (isDirty) { event.preventDefault(); }
    else if (forceOpen) { onClose(); }
  }, [config, initialConfig, onClose]);

  const Icon = getToolIcon(tool.id);
  const isConfigured = typeof config === 'object' && config?.config && Object.keys(config.config).length > 0;
